...
def main():
    ...
    with open("README.md", "r") as f:
        ...
        text = add_to_compat_table(leptos_version, bevy_version, crate_version_short, original_text)
...
def add_to_compat_table(leptos_version: str, bevy_version: str, crate_version: str, original_text: str):
    lines = original_text.splitlines()

    table_row = None

    if re.search(rf"^\|[^|]+\| {leptos_version}[^|]+\| {bevy_version}", lines[-1]) is not None:
        table_row = lines[-1]

    if table_row is None:
        lines.append(f"| {crate_version} | {leptos_version} | {bevy_version} |")
    elif re.search(rf"^\| (.* )?{crate_version}\s*\| {leptos_version}\s*\| {bevy_version}", table_row) is not None:
